...
using Content.Shared.Bed.Sleep; // A-13
...
namespace Content.Server.Speech.Muting
{
    public sealed class MutingSystem : EntitySystem
    {
        ...
        public override void Initialize()
        {
            ...
            SubscribeLocalEvent<SleepingComponent, ScreamActionEvent>(OnScreamActionSleeping, before: new[] { typeof(VocalSystem) }); // A-13 no screaming while unconscious
        }
        ...
        // A-13 no screaming while unconscious start
        private void OnScreamActionSleeping(EntityUid uid, SleepingComponent component, ScreamActionEvent args)
        {
            if (args.Handled)
                return;
            _popupSystem.PopupEntity(Loc.GetString("Вы не можете кричать без сознания."), uid, uid);
            args.Handled = true;
        }
        // A-13 no screaming while unconscious end
    }
}
